//import { AUDIO_STREAM_TYPE } from "./pull_demuxer_base";
...
export class AudioRenderer {
  ...
  async initialize(demuxer) {
    ...
    const configs = await this.demuxer.getDecoderConfigs();
    if (!configs.audio) {
      throw new Error('No audio track found in the file');
    }

    this.sampleRate = configs.audio.sampleRate;
    this.channelCount = configs.audio.numberOfChannels;

    debugLog('Audio config:', configs.audio);

    const support = await AudioDecoder.isConfigSupported(configs.audio);
    if (!support.supported) {
      throw new Error('Audio configuration not supported');
    }

    await this.decoder.configure(configs.audio);

    // Initialize ring buffer for decoded audio data
    const sampleCountIn500ms = DATA_BUFFER_DURATION * this.sampleRate * this.channelCount;
  ...
  async fillDataBufferInternal() {
    ...
    const usedBufferElements = this.ringbuffer.capacity() - this.ringbuffer.available_write();
    const usedBufferSecs = usedBufferElements / (this.channelCount * this.sampleRate);
    const pcntOfTarget = 100 * usedBufferSecs / DATA_BUFFER_DECODE_TARGET_DURATION;

    if (usedBufferSecs >= DATA_BUFFER_DECODE_TARGET_DURATION) {
      debugLog(`Buffer full - Used: ${usedBufferSecs}s (${pcntOfTarget}%)`);

      if (this.playing) {
        // Schedule refill when buffer is half empty
        setTimeout(() => this.fillDataBuffer(), 1000 * usedBufferSecs / 2);
      }
    ...
    // Keep decoding until buffer target is reached or decoder is saturated
    while (usedBufferSecs < DATA_BUFFER_DECODE_TARGET_DURATION &&
      this.decoder.decodeQueueSize < DECODER_QUEUE_SIZE_MAX) {

      const chunk = await this.demuxer.getNextChunk('audio');
      if (!chunk) {
        debugLog('No more audio chunks available');
        break;
      }

      this.decoder.decode(chunk);

      const currentBufferElements = this.ringbuffer.capacity() - this.ringbuffer.available_write();
      usedBufferSecs = currentBufferElements / (this.channelCount * this.sampleRate);
    }

    if (ENABLE_DEBUG_LOGGING) {
      const status = usedBufferSecs >= DATA_BUFFER_DECODE_TARGET_DURATION ?
        'buffered enough' : 'decoder saturated';
      debugLog(`Buffer status: ${status} - ${usedBufferSecs}s (${pcntOfTarget}%)`);
    }
  }
  ...
  interleave(inputs, inputOffset, inputSamplesToCopy, output, outputSampleOffset) {
    if (inputs.length * inputs[0].length < output.length) {
      throw new Error(`Not enough space in destination (${inputs.length * inputs[0].length} < ${output.length})`);
    }

    let outIdx = outputSampleOffset;
    let inputIdx = Math.floor(inputOffset / inputs.length);
    let channel = inputOffset % inputs.length;
  ...
  bufferAudioData(data) {
    // Initialize or resize interleaving buffers if needed
    if (this.interleavingBuffers.length !== data.numberOfChannels) {
      this.interleavingBuffers = Array(this.channelCount).fill(null)
        .map(() => new Float32Array(data.numberOfFrames));
    }

    debugLog(`Buffering audio data - Timestamp: ${data.timestamp}, Duration: ${data.duration / 1000000}s`);

    // Copy data to planar buffers
    for (let i = 0; i < this.channelCount; i++) {
      data.copyTo(this.interleavingBuffers[i], { planeIndex: i });
    }

    // Write to ring buffer
    const samplesWritten = this.ringbuffer.writeCallback(
      data.numberOfFrames * data.numberOfChannels,
      (firstPart, secondPart) => {
        this.interleave(this.interleavingBuffers, 0, firstPart.length, firstPart, 0);
        if (secondPart.length > 0) {
          this.interleave(this.interleavingBuffers, firstPart.length, secondPart.length, secondPart, 0);
        }
      }
    );

    // Check for buffer overflow
    if (samplesWritten !== data.numberOfChannels * data.numberOfFrames) {
      console.warn('Buffer full, dropping audio data!');
    }

    // Monitor buffer health
    const bufferHealth = this.bufferHealth();
    if (bufferHealth > this.maxBufferHealth) {
      this.maxBufferHealth = bufferHealth;
      debugLog(`New max buffer health: ${this.maxBufferHealth}%`);
    }

    // Continue filling buffer if needed
    this.fillDataBuffer();
  }
}
